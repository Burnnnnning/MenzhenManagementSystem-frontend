  ...
  <div>
    <el-row style="margin-bottom: 1px">
      ...
      <el-form :inline="true" style="margin-bottom: 20px">
        ...
        <el-form-item>
          <el-button type="primary" @click="handleEdit">查询</el-button>
        </el-form-item>
      </el-form>
    </el-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    async handleEdit() {
      try {
        console.log('handleEdit', this.searchForm.name);
        console.log('loading', this.loading);
        this.loading = true; // 显示加载状态
        // 调用 getDoctor 接口获取最新的医生信息
        const response = await api_doctor.getDoctor(this.searchForm.name);
        console.log('API Response: handle', response);
        if (response.code === 200) {
          this.dialogTitle = "编辑";
          this.form = { ...response.data }; // 使用接口返回的最新数据填充表单
          this.centerDialogVisible = true; // 显示对话框
        } else {
          this.$message.error('获取医生信息失败：' + response.message); // 提示失败
        }
      } catch (error) {
        this.$message.error('获取医生信息失败：' + error.message); // 提示失败
      } finally {
