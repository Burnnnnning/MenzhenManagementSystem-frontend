<template>
  <div>
    ...
    <el-row justify="space-between" align="middle">
      <!-- 添加患者信息按钮（左侧） -->
      <el-button type="success" @click="handleAdd" style="margin-top: -15px">添加患者信息</el-button>
    ...
    <div class="table-container" v-loading="loading">
      <el-table :data="tableData" border stripe style="width: 100%">
        ...
        <el-table-column prop="Patient_ID" label="患者编号" min-width="100"></el-table-column>
    ...
    <el-dialog v-model="centerDialogVisible" :title="dialogTitle" width="500" center>
      <el-form :model="form" label-width="100px">
        <el-form-item label="患者编号">
          <el-input v-model="form.Patient_ID" placeholder="请输入患者编号" :disabled="dialogTitle === '编辑'"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
      ...
      if (this.dialogTitle === "新增") {
        ...
        try {
          ...
          if (response.code === 200) {
            ...
            // 更新表格数据
            const index = this.tableData.findIndex(item => item.Patient_ID === this.form.Patient_ID);
            if (index !== -1) {
    ...
    async handleDelete(index, row) {
      try {
        this.loading = true; // 显示加载状态
        const response = await api_patient.deletePatient(row.Patient_ID); // 调用删除接口
        if (response.code === 200) {
