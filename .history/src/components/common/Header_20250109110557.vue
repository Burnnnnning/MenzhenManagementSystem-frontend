<template>
  <div class="header">
    ...
    <div class="nav">
      ...
      <router-link to="/manager" class="nav-button" active-class="active" @click="log" v-if="username.value !== 'admin'"
...
export default {
  ...
  setup() {
    ...
    // 调试输出
    const log = () => {
      console.log('调试输出: ', username.value);
    };

    // 监听登录事件
    onMounted(() => {
      EventBus.on('login', (newUsername) => {
        console.log('传递的用户名:', newUsername); // 打印传递的值
        username.value = newUsername; // 更新 username
        console.log('更新后的 username:', username.value); // 打印更新后的值
      });
    });
