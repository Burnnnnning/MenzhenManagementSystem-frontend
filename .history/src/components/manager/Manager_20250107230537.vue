    ...
    methods: {
        ...
        async search() {
            ...
            try {
                ...
                // 调用搜索 API
                const response = await api_login.searchUserLogin(keyword);
        ...
        async handleSave() {
            if (this.dialogTitle === "新增") {
                ...
                try {
                    this.loading = true; // 显示加载状态
                    // 调用更新用户登录接口
                    const response = await api_login.updateUserLogin(this.form);
                    if (response.code === 200) {
        ...
        async handleDelete(index, row) {
            try {
                this.loading = true; // 显示加载状态
                const response = await api_login.deleteUserLogin(row.User_Number); // 调用删除接口
                if (response.code === 200) {
        ...
        async loadTableData() {
            ...
            try {
                // 调用API获取所有用户登录数据
                const response = await api_login.getAllUserLogins();
