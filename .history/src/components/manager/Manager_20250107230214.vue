<template>
      ...
      <el-dialog v-model="centerDialogVisible" :title="dialogTitle" width="500" center>
        <el-form :model="form" label-width="100px">
          <el-form-item label="用户编号">
            <el-input v-model="form.User_Number" placeholder="请输入用户编号" :disabled="dialogTitle === '编辑'"></el-input>
          </el-form-item>
  ...
  <script>
  import api_user_login from '@/api/user_login'; // 导入用户登录 API 模块
      ...
      async handleSave() {
        if (this.dialogTitle === "新增") {
          try {
            this.loading = true; // 显示加载状态
            // 调用新增用户登录接口
            const response = await api_user_login.addUserLogin(this.form);
            if (response.code === 200) {
