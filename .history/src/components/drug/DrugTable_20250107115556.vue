<template>
    <div>
        ...
        <el-row justify="space-between" align="middle" style="margin-bottom: 1px">
            <!-- 添加药品信息按钮（左侧） -->
            <el-button type="success" @click="handleAdd" style="margin-top: -15px">添加药品信息</el-button>

            <!-- 搜索框和查询按钮（右侧） -->
            <el-form :inline="true">
                <el-form-item>
                    <el-input v-model="searchForm.name" placeholder="请输入药品名称"></el-input>
                </el-form-item>
                <el-form-item>
        ...
        <el-dialog v-model="centerDialogVisible" :title="dialogTitle" width="500" center>
            <el-form :model="form" label-width="100px">
                ...
                <el-form-item label="生产日期">
                    <el-input v-model="form.Drug_Production_Date" type="date" placeholder="请选择生产日期"></el-input>
                </el-form-item>
            </el-form>
            <template #footer>
...
export default {
    ...
    methods: {
        ...
        async handleDelete(index, row) {
            try {
                this.loading = true; // 显示加载状态
                const response = await api_drug.deleteDrug(row.Drug_Number); // 调用删除接口
                if (response.code === 200) {
