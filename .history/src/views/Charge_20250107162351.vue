...
import { getCharge } from '@/api/charge'; // 导入 getCharge 接口
...
const handlePatientSelect = async (patientId) => {
  ...
  if (selectedPatient) {
    ...
    try {
      // 调用 getCharge 接口获取费用信息
      const response = await getCharge(selectedPatient.idCard); // 假设 idCard 是患者的唯一标识
      const chargeData = response.data; // 假设返回的数据结构符合预期

      // 更新费用明细和金额
      form.feeDetails = chargeData.feeDetails || [];
      form.totalAmount = chargeData.totalAmount || 0;
      form.insuranceAmount = chargeData.insuranceAmount || 0;
      form.payAmount = chargeData.payAmount || 0;
    } catch (error) {
