...
import api_charge from '@/api/charge';
...
const handleSearch = () => {
  if (queryForm.patientName) {
    const response = api_patient.getPatientsByName(queryForm.patientName);
    console.log('Selected Patient:', response);
    // 处理查询逻辑，例如根据姓名查询患者列表，并更新 patientOptions
    patientOptions.value = response.map((patient) => ({
      value: patient.id,
      label: patient.name,
    }));
  } else {
...
const handlePatientSelect = async (patientId) => {
  ...
  if (selectedPatient) {
    ...
    // 调用API获取费用信息
    try {
      const response = await api_charge.getCharge(selectedPatient.id);
      const chargeData = response.data; // 假设返回的数据结构为 { feeDetails, totalAmount, insuranceAmount, payAmount }

      // 更新费用明细和结算信息
      form.feeDetails = chargeData.feeDetails;
      form.totalAmount = chargeData.totalAmount;
      form.insuranceAmount = chargeData.insuranceAmount;
      form.payAmount = chargeData.payAmount;
    } catch (error) {
