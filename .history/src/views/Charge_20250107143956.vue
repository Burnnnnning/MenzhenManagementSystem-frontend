<template>
    <div class="settlement-page">
      ...
      <el-form :model="queryForm" label-width="120px" class="query-form">
        <el-form-item label="患者姓名">
          <el-input v-model="queryForm.patientName" placeholder="请输入患者姓名" @input="handleSearch" />
        </el-form-item>
      ...
      <!-- 费用结算表单 -->
      <el-form :model="form" label-width="120px" class="settlement-form">
        <!-- 患者信息 -->
        <el-form-item label="患者姓名">
        ...
        <!-- 费用明细 -->
        <el-table :data="form.feeDetails" style="width: 100%">
  ...
  const handleSearch = () => {
    if (queryForm.patientName) {
      patientOptions.value = mockPatients.filter(
        (patient) => patient.name.includes(queryForm.patientName)
      );
    } else {
