...
export default {
  ...
  methods: {
    ...
    async handlePatientSelect(patientNumber) {
      ...
      if (selectedPatient) {
        // 更新患者基本信息
        this.form.Patient_Number = patientNumber;
        ...
        try {
          ...
          if (this.form.Medical_Insurance === '有') {
            ...
            this.form.insuranceAmount = parseFloat((this.form.totalAmount * 0.6).toFixed(2));
            this.form.payAmount = this.form.totalAmount - this.form.insuranceAmount;
          } else {
    ...
    async handleSettlement() {
      ...
      const response = await api_charge.SettleCharge(this.form.Patient_Number);
