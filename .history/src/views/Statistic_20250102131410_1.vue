...
export default defineComponent({
    ...
    setup() {
        ...
        const distributeDataToCounties = (data) => {
            ...
            if (!totalData) {
                console.error('totalData is undefined or null');
                return [];
            }

            if (counties.length === 0) {
                console.error('counties is empty');
                return [];
            }
            ...
            ['waterResource', 'availableResource', 'usableWater', 'actualUsage'].forEach(key => {
                ...
                countyData[counties.length - 1][key] += (totalData[key] || 0) - total;
            });
            

            ['waterResource', 'availableResource', 'usableWater', 'actualUsage'].forEach(key => {
                if (countyData.length === 0) {
                    console.error('countyData is empty');
                    return;
                }
                ...
                // 计算差值并累加到最后一个县区
                countyData[countyData.length - 1][key] = (countyData[countyData.length - 1][key] || 0) + (totalData[key] || 0) - total;

                // 调试日志
                if (process.env.NODE_ENV === 'development') {
                    console.log(`${key}:`, countyData.map(c => c[key]));
                }
            });
