      ...
      if (selectedPatient) {
        ...
        try {
          ...
          this.form.feeDetails = [
            { item: '挂号费', price: chargeData.registration_fee, quantity: 1, amount: chargeData.registration_fee },
            { item: '药品费', price: chargeData.drug_fee, quantity: 1, amount: chargeData.drug_fee },
            { item: '治疗费', price: chargeData.treatment_fee, quantity: 1, amount: chargeData.treatment_fee },
          ];
          this.form.totalAmount = chargeData.total_fee || 0;
          this.form.insuranceAmount = 0; // 假设医保报销金额为 0，根据实际情况修改
          this.form.payAmount = chargeData.total_fee || 0; // 假设自费金额等于总费用，根据实际情况修改
        } catch (error) {
